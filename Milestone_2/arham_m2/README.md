# Arham Milestone 2 — Distributed LSH Index Construction
...
- LSH automatically uses Raviha's `ConsistentHashRing` if available; falls back to modulo partitioning.
- Index construction is parallelized across threads (CPU-bound).
- Query latency depends on table parameters and data distribution.
- Next milestone: Fatima will implement distributed query fanout and result aggregation.
